/*
Plugin_orb - A Phaser Plug in that helps with the creation of Orb Sprites.
 */
var Plugin_Orb = function (game, options) {

    // if not game is given assume one is attached to window
    ...
    // make the basic 'sheet-orb' sprite sheet used for development
    var mkOrbSheet = function (game) {
        ...
        game.cache.addSpriteSheet('sheet-orb', null, canvas, 32, 32, 1, 0, 0);
    ...
    plug.init = function () {
        ...
        console.log('Plugin_Orb: init');

        // make 'sheet-orb' sprite sheet
        mkOrbSheet(game);

        // add CreateOrbSprite
        game.data.CreateOrbSprite = function (opt) {

            opt = opt || {};
            opt.x = opt.x === undefined ? 0 : opt.x;
            opt.y = opt.y === undefined ? 0 : opt.y;

            var orbSprite = game.make.sprite(opt.x, opt.y, 'sheet-orb', 0);
